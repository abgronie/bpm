...
public class BpmFormDefService extends BaseService<BpmFormDef> {
	...
	public void updateForm(BpmFormDef bpmFormDef) throws Exception {
		BpmFormDef oldBpmFormdef = this.getById(bpmFormDef.getFormDefId());
		String oldHtml = oldBpmFormdef.getHtml();
		// 更新table
		dao.update(bpmFormDef);

		historyDataService.add(SysHistoryData.SYS_FORMDEF_TEMPLATE, bpmFormDef.getSubject(), oldHtml, bpmFormDef.getFormDefId());
	}
	...
	public String parseHtml(BpmFormDef bpmFormDef, String pkValue, String instanceId, String actDefId, String nodeId, String parentActDefId, boolean isReCalcute, boolean isPreView, boolean isReadOnly,boolean isCopy) throws Exception {
		...
		html = bpmFormHandlerService.getTabHtml(bpmFormDef.getTabTitle(), bpmFormDef.getHtml(),permission);
		html = BpmFormDefHtmlParser.parseTeam(html, param);
		html = BpmFormDefHtmlParser.parse(html, param);
		return html;
	}
}