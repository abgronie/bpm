...
public class BpmFormRightsService {
	...
	private Map<String, Object> getDefaultPermissionByFormKey(BpmFormDef bpmFormDef, boolean isReadonly) {
		...
		//5分组默认权限
		List<JSONObject> teamPermissionJson = new ArrayList<JSONObject>();
		if(BeanUtils.isNotIncZeroEmpty(bpmFormDef.getTableId())){
			teamPermissionJson = getTeamTableJson(bpmFormDef);
		}
	...
	private List<JSONObject> getTeamJsonList(List<BpmFormRights> teamTableRightsList,BpmFormDef bpmFormDef){
		...
		if(BeanUtils.isNotEmpty(bpmFormTable.getTeam())){
			JSONObject jsonObject = JSONObject.fromObject(bpmFormTable.getTeam());
