...
public class WeiXinMessageHandler implements IMessageHandler {
	...
	public void handMessage(MessageModel model) {
		logger.error("----------handMessageWeixin-------------start");
		SysUser recever = (SysUser) model.getReceiveUser();
		logger.error("----------recever-------------"+recever.getAccount());
//		微信推送消息是通过应用推送消息，不需要发送人
//		if ( model.getSendUser() == null) return;
		// 没有微信号就不发了
		if(recever == null || recever.getHasSyncToWx()==0) return; 
		String weiXinId = recever.getWeixinid();
		logger.error("----------weiXinId-------------"+weiXinId);
		if(StringUtil.isNotEmpty(weiXinId)){
			sendMessage(MessageUtil.getContentForWx(model),weiXinId);
		}
	}
