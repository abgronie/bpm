...
public class SysPropertyService extends BaseService<SysProperty> implements IPropertyService {
	 ...
	 * @return
	 */
	public Long getIdByAlias(String alias) {
		return dao.getByAlias(alias).getId();
	}

	public SysProperty getPropertyByAlias(String alias){
		SysProperty sysProperty = dao.getByAlias(alias);
		return sysProperty;
	}
	 ...
	 */
	public void updateProperty(WeChatParameter weChatParameter){
		Map<String, String> map = weChatParameter.toMap();
		List<String> keyList = new ArrayList<String>(map.keySet());
		for(String key : keyList){
			SysProperty sysProperty = getPropertyByAlias(key);
			sysProperty.setValue(map.get(key));
			save(sysProperty);
		}
	}
