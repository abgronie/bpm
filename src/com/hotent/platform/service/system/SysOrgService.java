...
import com.hotent.weixin.impl.WxUserService;
...
public class SysOrgService extends BaseService<SysOrg>{
	...
	private com.hotent.weixin.api.IUserService wxUserService;
	...
	public void delById(Long id) {
		...
		//删除组织分配的角色 
				for(int i=0;i<sysOrgs.size();i++){
					//EventUtil.publishDepartmenUserEvent(id, UserEvent.ACTION_DEL,sysOrg);
					//删除组织分配的角色
	...
	private void delWeiXinOrg(SysOrg sysOrg) {
		String path = sysOrg.getPath();
		path = StringUtil.isNotEmpty(path)?(path+"%"):"";
		List<SysOrg> orgList =dao.getByOrgPath(path);
		List<SysOrg> orgChildrenList = new ArrayList<SysOrg>();
		
		for(SysOrg org : orgList ){
			if(!(org.getOrgId().equals(sysOrg.getOrgId()))){
				orgChildrenList.add(org);
			}
		}
		
		//判断子组织是否存在
		if(orgChildrenList.size()>0){
			for(SysOrg orgChildren : orgChildrenList ){
				delWeiXinOrg(orgChildren);
			}
		}else{
			//根据组织微信CODE获取到改组织下的所有成员
			String userAccout = wxOrgService.getDepartmentUser(sysOrg.getCode());
			//先删除组织下的用户（企业微信端）
			wxUserService.deleteAll(userAccout);
			//删除组织
			wxOrgService.delete(sysOrg.getCode());
		}	  
	}
