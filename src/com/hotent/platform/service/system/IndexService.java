...
public class IndexService {
	...
	public JSONObject getUserInfo() {
		...
		if(BeanUtils.isNotEmpty(org)){
			userInfo.put("org", JSONObject.fromObject(org));
		}
		Position position = (Position) ContextUtil.getCurrentPos();
		if(BeanUtils.isNotEmpty(position)){
			userInfo.put("pos", JSONObject.fromObject(position));
		}
		return userInfo;
	}
	 ...
	 * @return
	 */
	public List<SysBulletin> getBulletin() {
		List<SysBulletin> list = sysBulletinService.getMyHasRightsBulletin();
		return list;
	}
	...
	public String myCalendar() {
		...
		IndexService indexService = (IndexService)AppUtil.getBean("indexService");
		Integer curPage = 1;
		Integer pageSize = 100;
		PageList<ProcessTask> pageList = indexService.pendingMatterPage(curPage, pageSize);
		//pendingMatterPage
		for(ProcessTask task:pageList){
		JSONObject jsonObj = new JSONObject();
		jsonObj.accumulate("title",task.getSubject())
		.accumulate("start",DateFormatUtil.format(task.getCreateTime(),"yyyy-MM-dd"))
		.accumulate("backgroundColor", "#70AFC4")
		.accumulate("eventClick", "function(){jQuery.openFullWindow(__ctx+'/platform/bpm/task/toStart.ht?taskId="+task.getId()+"');}");
		jsonAry.add(jsonObj);
		}
