...
public class ProcessRunService extends BaseService<ProcessRun> {
	...
	public ProcessRun nextProcess(ProcessCmd processCmd) throws Exception {
		...
		Map<String, String> revokedMap = null;
		if (parentStack != null) {
			parentNodeId = parentStack.getNodeId();
			// 判断是否撤销而不是驳回
			if(processCmd.isRecover()){
				revokedMap = this.getTempByUseType(SysTemplate.USE_TYPE_REVOKED);
			}
		}
		// 设置会签用户或处理会签意见
		...
		taskMessageService.notify(TaskThreadService.getNewTasks(), informType, processRun.getSubject(), revokedMap, processCmd.getVoteContent(), parentActDefId);
		// 处理代理
		...
		if (StringUtil.isNotEmpty(action)) {
			...
			dataMap.put("startUser",processRun.getCreatorId());
	...
	// 根据cmd的审批
	public String getActionByCmdVoteAgree(Short sot) {
		if (sot == 1 || sot == 5)
	 ...
	 * @throws Exception
	 */
	private Boolean skipTask(Task task, ProcessCmd processCmd) throws Exception {
		Boolean tag = false;
	...
	public ProcessRun startProcess(ProcessCmd processCmd) throws Exception {
		...
		// 启动流程后，处理业务表单，外部调用不触发表单处理,如果是URL表单，则跳过
		if(bpmNodeSet.getFormType()==0){
		...
		Boolean handJumpOverFirstNodeResult = false;

		// 获取下一步的任务并完成跳转。
		if (toFirstNode == 1) {
			handJumpOverFirstNodeResult = handJumpOverFirstNode(processInstanceId, processCmd);
		}
		...
		// 如果跳过了第一个节点，则不需要再发送消息
		if(!handJumpOverFirstNodeResult){
			// 处理信息
	 ...
	 * @throws Exception
	 */
	private Boolean handJumpOverFirstNode(String processInstanceId, ProcessCmd processCmd) throws Exception {
		...
		return skipTask(taskEntity, processCmd);
	...
	private  void handFieldOpinion(ProcessCmd processCmd, BpmFormData bpmFormData) throws Exception {
		...
		//@byq 用batComplteByq字段判断是否批量审批（因为字段默认为大写，而这里小写所以重复插入了意见填写字段）
		opinionField = opinionField.toLowerCase();
		boolean batComplteByq = processCmd.getTransientVar("batComplte")!=null?(Boolean)processCmd.getTransientVar("batComplte"):false;//批量审批标记
		if(batComplteByq){
			opinionField = opinionField.toUpperCase();
		}
		...
		if(mainMap.containsKey(opinionField)){
			...
			//@byq opinion_log 设置意见回填是否保持历史记录(为1的时候保持历史意见记录) @author byq
			String opinionLog = sysPropertyService.getByAlias("opinion_log", "1");

			if ((StringUtil.isNotEmpty(str)) && "1".equals(opinionLog)) {
				val = str + "<p>"+ val +"</p>";
			}
		}
	 ...
	 * @throws Exception
	 */
	public boolean saveCommuniCation(TaskEntity taskEntity, String opinion, String informType, String userIds, String subject) throws Exception {
		String taskId = taskEntity.getId();
	 ...
	 * @return
	 */
	public List<ProcessRun> getAlreadyMattersWithSamePositionList(QueryFilter filter) {
		...
		filter.addFilter("userPositions", userPositions);

		return dao.getAlreadyMattersWithSamePositionList(filter);
	}
	 ...
	 * @return
	 */
	public List<ProcessRun> getAlreadyMattersByjlList(QueryFilter filter) {
		return dao.getAlreadyMattersByjlList(filter);
	}
	...
	public void saveData(ProcessCmd processCmd) throws Exception {
		...
		// 处理业务表单，外部调用不触发表单处理
		if (!processCmd.isInvokeExternal() && bpmNodeSet.getFormType()==bpmNodeSet.FORM_TYPE_ONLINE) {
	 ...
	 */
	// TODO (重写方法为了新办事大厅)
	public ResultMessage checkRecover(Long runId, Long userId) throws Exception {
		ProcessRun processRun = this.getById(runId);
		if (processRun.getParentId() != null && processRun.getParentId() != 0) {
			processRun = this.getById(processRun.getParentId());
		}
		...
		ResultMessage messag = new ResultMessage(ResultMessage.Success, "可以撤销!");
		if (!isCanRecover(processRun,  messag, userId)) {
			return messag;
		}
		return messag;
	}
	 ...
	 */
	// TODO (重写方法为了新办事大厅)
	private List<ProcessTask> getLastTaskId(Long actInstId,ResultMessage resultMsg,Long curUserId){
		List<ProcessTask> list=new ArrayList<ProcessTask>();
		ProcessTaskHistory taskEntity = taskHistoryDao.getLastFinshTaskByProcId(actInstId,curUserId);
		// 根据流程实例Id获取堆栈列表。
		List<ExecutionStack> stackList=executionStackService.getByActInstId(actInstId);
	 ...
	 */
	// TODO (重写方法为了新办事大厅)
	private Boolean isSignTaskRecover(Long actInstId, Long curUserId){
		ProcessTaskHistory taskEntity = taskHistoryDao.getLastFinshTaskByProcId(actInstId,curUserId);
		String processDefinitionId = taskEntity.getProcessDefinitionId();
		String taskDefinitionKey = taskEntity.getTaskDefinitionKey();
		FlowNode flowNode = NodeCache.getByActDefId(processDefinitionId).get(taskDefinitionKey);
	...
	private Boolean isCanRecover(ProcessRun processRun,  ResultMessage message) {
		...
		Long userId=ContextUtil.getCurrentUserId();
		if(userId == 0){
			userId = processRun.getCreatorId();
		}
		//获取最近的审批任务。
	 ...
	 */
	// TODO (重写方法为了新办事大厅)
	private Boolean isCanRecover(ProcessRun processRun, ResultMessage message, Long userId) {
		BpmDefinition bpmDefinition = bpmDefinitionDao.getByActDefId(processRun.getActDefId());
		if (BpmDefinition.STATUS_INST_DISABLED.equals(bpmDefinition.getStatus())) {
			message.setMessage("对不起,此流程定义已禁用实例!");
			message.setResult(ResultMessage.Fail);
		...
		Boolean signTaskRecover = isSignTaskRecover(actInstId, userId);
		//会签任务允许撤销
		if(signTaskRecover){
			return true;
		}
		List<ProcessTask> rtnList = getLastTaskId(actInstId, message, userId);
		if(message.getResult()==ResultMessage.Fail){
			return false;
		}
		boolean hasRead = getTaskHasRead(rtnList);
...
//			}
		...
		if (!isExtForm) {// 在线表单
			...
			////@author byq isReadOnly 是否只读，如果设为true 那么如果有写权限的就会认为是只读，否则按照权限设定来走。
	 ...
	 * @return
	 */
	public List<ProcessRun> getSxtjforten(Map<String,Object> params, PageBean pb) {
		return dao.getSxtjforten(params, pb);
	}
	 ...
	 * @return
	 */
	public List<ProcessRun> getZdsjtj(Map<String,Object> params) {
		return dao.getZdsjtj(params);
	}
	 ...
	 * @return
	 */
	public List<ProcessRun> getSxbltj(Map<String,Object> params,PageBean pb) {
		return dao.getSxbltj(params,pb);
	}
	 ...
	 * @return
	 */
	public List<ProcessRun> getNodeAvgTime(Map<String,Object> params) {
		return dao.getNodeAvgTime(params);
	}
	/**
	 * 人员超时统计
	 * @param params
	 ...
	 * @return
	 */
	public List<ProcessRun> getRycstj(Map<String,Object> params, PageBean pb){
		return dao.getRycstj(params,pb);
	}
	 ...
	 * @return
	 */
	public List<ProcessRun> getSxbltjxq(Map<String,Object> params, PageBean pb){
		return dao.getSxbltjxq(params, pb);
	}
