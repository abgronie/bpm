...
public class TaskCreateListener extends BaseTaskListener {
	...
	private void handAssignUserFromDb(String actDefId,String nodeId,DelegateTask delegateTask){
		...
		if(processInstance!=null){
			...
			String startUserId=String.valueOf(vars.get(BpmConst.StartUser));
			...
			if(BeanUtils.isEmpty(taskExecutors)){
				...
				//否则，从数据库获取人员设置
					...
					if(processInstance!=null){
						...
						String actInstId=delegateTask.getProcessInstanceId();
						String startUserId=String.valueOf(delegateTask.getVariable(BpmConst.StartUser));
	...
	private void assignUser(DelegateTask delegateTask, List<TaskExecutor> executors){
		if(BeanUtils.isEmpty(executors)){
			...
			Object transientVar = cmd.getTransientVar(BpmConst.BPM_DEF);
			if(transientVar!=null && transientVar instanceof IBpmDefinition){
				bpmDefinition = (BpmDefinition)transientVar;
			}
			if(bpmDefinition==null){
				String actDefId = cmd.getActDefId();
				if(StringUtil.isNotEmpty(actDefId)){
					bpmDefinition = bpmDefinitionDao.getByActDefId(actDefId);
				}
				else{
					String flowKey = cmd.getFlowKey();
					if(StringUtil.isNotEmpty(flowKey)){
						bpmDefinition = bpmDefinitionDao.getMainByDefKey(flowKey);
					}
				}
