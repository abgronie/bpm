...
public class TaskApprovalItemsService extends BaseService<TaskApprovalItems>
{
			...
			String  defKeyArr, Long curUserId) throws Exception
	{
		...
		if (type==1||type==4||type==8||type==9||type==10) {
	 ...
	 * @return
	 */
	public List<String> getApprovalByDefKeyAndTypeIdBtn(String defKey,Long typeId,String type){
		List<String> taskAppItemsList = new ArrayList<String>();
		Long curUserId=ContextUtil.getCurrentUserId();
		...
		List<TaskApprovalItems> taskAppItem = new ArrayList<TaskApprovalItems>();
		if("8".equals(type)){
			//获取分类为8的所有的常用语
			taskAppItem=dao.getByType(TaskApprovalItems.TYPE_btnAgree);
		}if("9".equals(type)){
			//获取分类为9的所有的常用语
			taskAppItem=dao.getByType(TaskApprovalItems.TYPE_btnEnd);
		}if("10".equals(type)){
			//获取分类为10的所有的常用语
			taskAppItem=dao.getByType(TaskApprovalItems.TYPE_btnReject);
		}
		if (BeanUtils.isNotEmpty(taskAppItem)) {
			for(TaskApprovalItems taskApp:taskAppItem){
				taskAppItemsList.add(taskApp.getExpression());
			}
		}
		...
		if (BeanUtils.isNotEmpty(taskAppItem2)) {
			...
			String[] typeIds=typeIdPath.split("\\.");
			for (int i=1; i< typeIds.length ;i++) {
				for (TaskApprovalItems taskItem:taskAppItem2) {
					if ((taskItem.getTypeId().toString()).equals(typeIds[i])) {
						taskAppItemsList.add(taskItem.getExpression());
					}
				}
			}
		}
