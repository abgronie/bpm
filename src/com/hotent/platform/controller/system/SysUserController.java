...
public class SysUserController extends BaseController {
	...
	public ModelAndView list(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		// 是否同步至微信
		boolean isSupportWeixin = PropertyUtil.getBooleanByAlias(SysPropertyConstants.WX_IS_SUPPORT,true);
	...
	public ModelAndView dialog(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		Boolean ifShowChildNode= true;//根据用户和其表单的选择器类型，判断该选择器的组织是否含有子节点，以便初始化样式
		Boolean isLeader = false;
		Boolean isNext = false;
		if ( StringUtil.isNotEmpty(type) && "system".equals(type)) {			
			if ("self".equals(typeVal)) {//如选本级和下级。则取判断该用户所在组织是否有下级组织
				SysOrg sysOrg= orgServiceImpl.getSysOrgByScope(type, typeVal);
				List<SysOrg> orgList = sysOrgService.getOrgByOrgSupId(sysOrg.getOrgId());
					if (orgList.size() <1) ifShowChildNode =false;
				}else if ("department".equals(typeVal) || "up".equals(typeVal)) {//本部，上级。都只包含该级
					ifShowChildNode =false;
				}
				//<!-- 计量校领导 -->
				else if ("leader".equals(typeVal)) {//本部，上级。都只包含该级
					isLeader = true;
				}
				else if ("next".equals(typeVal)) {//本部，上级。都只包含该级
					isNext = true;
				}
		}
		String isSingle = RequestUtil.getString(request, "isSingle", "false");
		mv.addObject("isNext", isNext).addObject("isLeader", isLeader).addObject("isSingle", isSingle).addObject("ifShowChildNode", ifShowChildNode);
		handelUserSoruce(mv);
		return mv;
	}
	...
	public ModelAndView selector(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (SystemConst.SEARCH_BY_ONL.equals(searchBy)) {
		...
		} else if (SystemConst.SEARCH_BY_NAME.equals(searchBy)) {
			//按工号 @author byq
			String account = RequestUtil.getString(request, "account");
			queryFilter.addFilter("account",  account);
			//按姓名 @author byq
			String userName = RequestUtil.getString(request, "userName");
			userName = URLDecoder.decode(userName, "UTF-8");
			queryFilter.addFilter("fullname",  "%" + userName + "%");
			list = sysUserService.getUserByUserName(queryFilter);
		} else {
			...
			if (StringUtil.isNotEmpty(type) && !"all".equals(typeVal) && BeanUtils.isNotEmpty(sysOrg)) {
				SysOrg filterOrg= orgServiceImpl.getSysOrgByScope(type, typeVal);
				if ( "department".equals(typeVal) || "up".equals(typeVal)) {//如果是只选本部门或者只选上级
					queryFilter.addFilter("orgId", filterOrg.getOrgId());
					queryFilter.addFilter("searchBy", 2);
					list = sysUserService.getDistinctUserByOrgId(queryFilter);

				}else{
					queryFilter.addFilter("path", filterOrg.getPath() + "%");
					list = sysUserService.getDistinctUserByOrgPath(queryFilter);
				}
			//<!-- 计量校领导 -->
			}else if( "leader".equals(typeVal)){
				queryFilter.addFilter("orgId", PropertyUtil.getByAlias("leaderId"));
				list = sysUserService.getDistinctUserByOrgId(queryFilter);
			} else {
