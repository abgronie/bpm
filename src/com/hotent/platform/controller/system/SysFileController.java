...
import com.hotent.platform.controller.system.util.Word2Html;
...
public class SysFileController extends BaseController {
	...
	public void getFile(HttpServletRequest request, HttpServletResponse response, Long fileId) throws IOException {
		...
		if(os.toLowerCase().startsWith("win")){  	
		  ...
		  wordPath = attachPath+"\\"+file.split("\\\\")[0];
		  wordName = file.split("\\\\")[1].split("\\.")[0];
		  truePath = request.getSession().getServletContext().getRealPath("")+File.separator+"html";
		}else {
		  wordPath = attachPath+"/"+file.split("\\/")[0];
		  wordName = file.split("\\/")[1].split("\\.")[0];
		  truePath = request.getSession().getServletContext().getRealPath("")+File.separator+"html";
		 ...
		 String serverUrl = sysPropertyService.getByAlias("serverUrl", "0");
		 ...
		 if(StringUtils.isNotBlank(isDownload)) {
			 ...
			 if(suffix.equals("doc")) {
				 w2h.Word2003ToHtml(wordPath, wordName, "."+suffix,truePath);
				 response.sendRedirect(serverUrl+"/html/"+wordName+"_show/"+wordName+".html");
			 }else if (suffix.equals("docx")) {
				 w2h.Word2007ToHtml(wordPath, wordName, "."+suffix,truePath);
				 response.sendRedirect(serverUrl+"/html/"+wordName+"_show/"+wordName+".html");
			 }else {
				try{
					String fileName = sysFile.getFileName() + "." + sysFile.getExt();
