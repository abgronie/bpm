...
public class SysBulletinController extends BaseController {
	...
	public ModelAndView list(HttpServletRequest request,
		...
		Long cusUserId = ContextUtil.getCurrentUserId();
		QueryFilter filter = new QueryFilter(request, "sysBulletinItem");
		if(!isSuperAdmin){
			filter.addFilter("cusUserId", cusUserId);
		}
		...
		ModelAndView mv = this.getAutoView().addObject("sysBulletinList", list)
				...
				.addObject("currentUserId", cusUserId)
				.addObject("objType",SysObjRights.RIGHT_TYPE_SYS_BULLETIN);
		return mv;
	}
