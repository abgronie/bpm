...
public class SysPropertyController extends BaseController {
	 ...
	 * @throws Exception
	 */
	@RequestMapping("changeHomeStyle")
	@Action(description = "切换主页风格")
	public void changeHomeStyle(HttpServletRequest request, HttpServletResponse response) throws Exception {
		String style = RequestUtil.getString(request, "style");
		long uiStyleId = RequestUtil.getLong(request, "uiStyleId");
		String resultMsg = null;
		try {
			SysProperty sysProperty = sysPropertyService.getById(uiStyleId);
			sysProperty.setValue(style);
			sysPropertyService.save(sysProperty);
			resultMsg = getText("切换", "主页风格");
			writeResultMessage(response.getWriter(), resultMsg, ResultMessage.Success);
		} catch (Exception e) {
			writeResultMessage(response.getWriter(), resultMsg + "," + e.getMessage(), ResultMessage.Fail);
		}
