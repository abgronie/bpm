...
public class BpmDataTemplateController extends BaseController {
	...
	public ModelAndView edit(HttpServletRequest request) throws Exception {
		...
		if (BeanUtils.isEmpty(bpmDataTemplate)) {
			...
			bpmDataTemplate.setDisplayField(this.getDisplayField(bpmFormTable, "",formViewFiledMap));

			bpmDataTemplate.setExportField(this.getExportField(bpmFormTable, "",formViewFiledMap));
		} else {
			...
			bpmDataTemplate.setDisplayField(this.getDisplayField(bpmFormTable, bpmDataTemplate.getDisplayField(),formViewFiledMap));

			bpmDataTemplate.setExportField(this.getExportField(bpmFormTable, bpmDataTemplate.getExportField(),formViewFiledMap));
		}
		...
		//获取表单内有定义的主表字段
		List<BpmFormField> formViewMainFields = getFormViewMainFields(bpmFormTable,formViewFiledMap);
		
		JSONObject jsonObject=tidyJson(bpmDataTemplate);
		
		return this.getAutoView().addObject("bpmFormTableJSON", JSONObject.fromObject(bpmFormTable)).addObject("formViewMainFields", JSONArray.fromObject(formViewMainFields))
	...
	List<BpmFormField> getFormViewMainFields(BpmFormTable bpmFormTable,Map<String,Set<String>> formViewFiledMap){
		List<BpmFormField> bpmFormFields = new ArrayList<BpmFormField>();
		Set<String> mainFields = formViewFiledMap.get(bpmFormTable.getTableName());
		for(BpmFormField bpmFormField : bpmFormTable.getFieldList()){
			String fieldName = bpmFormField.getFieldName();
			if(mainFields.contains(fieldName)){
	...
	//显示字段	
	private String getDisplayField(BpmFormTable bpmFormTable, String displayField,Map<String,Set<String>> formViewFiledMap) {
		...
		if (BeanUtils.isNotEmpty(bpmFormTable)) {
			...
			for (BpmFormField bpmFormField : fieldList) {
				if(!mainFields.contains(bpmFormField.getFieldName())){
					continue;
				}
	...
	//====end 显示字段	
	//==== 导出字段	
	private String getExportField(BpmFormTable bpmFormTable, String field,Map<String,Set<String>> formViewFiledMap ) {
		...
		List<BpmFormField> fieldList = bpmFormTable.getFieldList();
		jsonAry.add(getTableField(bpmFormTable, fieldList, map, descMap,formViewFiledMap));
		List<BpmFormTable> subTableList = bpmFormTable.getSubTableList();
		for (BpmFormTable subTable : subTableList) {
			jsonAry.add(getTableField(subTable, subTable.getFieldList(), map, descMap,formViewFiledMap));
		}

		return jsonAry.toString();
	}

	private JSONObject getTableField(BpmFormTable bpmFormTable, List<BpmFormField> fieldList, Map<String, String> map, Map<String, String> descMap,Map<String,Set<String>> formViewFiledMap) {
		...
		for (BpmFormField bpmFormField : fieldList) {
			...
			String desc = bpmFormField.getFieldDesc();
			if (BeanUtils.isNotEmpty(descMap) && descMap.containsKey(key)) {
	...
	public void exportData(HttpServletRequest request, HttpServletResponse response, @PathVariable(value = "alias") String alias) throws Exception {
		...
		ExcelUtil.downloadExcel(wb, fileName, request,response);
	}
	 ...
	 * @throws Exception
	 */
	@RequestMapping("exportPic_{alias}")
	@Action(description = "导出业务数据模板图片", detail = "导出业务数据模板图片")
	public void exportPic(HttpServletRequest request, HttpServletResponse response, @PathVariable(value = "alias") String alias) throws Exception {
		Map<String, Object> params = RequestUtil.getQueryMap(request);

		String[] exportIds = RequestUtil.getStringAryByStr(request, "__exportIds__");
		int exportType = RequestUtil.getInt(request, "__exportType__");
		bpmDataTemplateService.export(alias, exportIds, exportType, params, request, response);
	}
	...
	public ModelAndView importData(HttpServletRequest request, HttpServletResponse response, @PathVariable(value = "alias") String alias) throws Exception {
		ModelAndView mv = new ModelAndView();
		//BpmDataTemplate bpmDataTemplate = bpmDataTemplateService.getByFormKey(alias);
		//List<BpmFormField> fieldList = bpmFormFieldService.getByTableId(bpmDataTemplate.getTableId());
	...
	public void importSave(MultipartHttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String importIdField = RequestUtil.getString(request, "importIdField");
		String emptyDo = RequestUtil.getString(request, "emptyDo");
		MultipartFile file = request.getFile("file");
		ResultMessage message = null;
		try {
			bpmDataTemplateService.importFile(file.getInputStream(), alias, importType,importIdField,emptyDo);
			message = new ResultMessage(ResultMessage.Success, "导入成功!");
		} catch (Exception e) {
			e.printStackTrace();
			message = new ResultMessage(ResultMessage.Fail, "导入出错了，请检查导入格式是否正确或者导入的数据是否有问题！" + e.getMessage());
		}
