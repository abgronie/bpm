...
public class BpmGangedSetController extends BaseController
{
	...
	{	
		...
		if(StringUtil.isEmpty(parentActDefId)) {
		 ...
		 if(BeanUtils.isNotEmpty(globalNodeSet)){//取出该表单，防止在表单里面设置了自定义查询，只取表单节点，取不到相关设置的问题
			 bpmFormDef = bpmFormDefService.getDefaultVersionByFormKey(globalNodeSet.getFormKey());
