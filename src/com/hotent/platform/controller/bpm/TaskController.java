...
public class TaskController extends BaseController {
	...
	public ModelAndView startFlowForm(HttpServletRequest request, HttpServletResponse response, Long defId) throws Exception {
		...
		if (runId != 0) {
			...
			defId = processRun.getDefId();
			//@author byq
			if(!sysUser.getUserId().equals(processRun.getCreatorId())){
		...
		if (BeanUtils.isNotEmpty(processRun) && processRun.getStatus().equals(ProcessRun.STATUS_FORM)) {
			...
			currentNode = bpmNodeSet.getNodeId();
	...
	public void startFlow(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			String flag = AppConfigUtil.get("messageflag");
			if("true".equals(flag)){
				ChannelUtils channelUtils = new ChannelUtils();
				channelUtils.test(curUser.getAccount(), "1");
				// TODO 消息（从草稿申请）
				if (runId != 0L) {
					channelUtils.test(curUser.getAccount(), "3");
				}
			}
	...
	public void toStartCommunication(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			//@byq 沟通人不能是当前审批人
			boolean flag = processRunService.saveCommuniCation(taskEntity, opinion, informType, cmpIds, processRun.getSubject());
			if(flag){
				ProcessCmd taskCmd = BpmUtil.getProcessCmd(request);
				...
				resultMessage = new ResultMessage(ResultMessage.Fail, "失败!请重新选择沟通人员!不要选择当前审批人!");
			}
		} catch (Exception e) {
	...
	public void complete(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (ServiceUtil.isAssigneeNotEmpty(assignee) && !task.getAssignee().equals(userId.toString()) && !isAdmin) {
			...
			try {
				...
				if("true".equals(flag)){
					ChannelUtils channelUtils = new ChannelUtils();
					channelUtils.test(curUser.getAccount(), "2");
				}
	...
	public ModelAndView pendingMattersList(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		/*if(filter.getFilters().get("creator") != null){
			String creator = filter.getFilters().get("creator").toString();
			if (StringUtils.isNotEmpty(creator))
				filter.getFilters().put("creator", "%" + creator + "%");
		}*/
	...
	public ModelAndView toStartCommunicate(HttpServletRequest request) throws Exception {
		...
		String cmpIds = communicateUser.get("cmpIds").toString();
		String cmpNames = communicateUser.get("cmpNames").toString();
		Map<String, IMessageHandler> handlersMap = ServiceUtil.getHandlerMap();
		return getAutoView().addObject("handlersMap", handlersMap)
	...
	@RequestMapping("opDialogBtn")
	@Action(description = "填写意见(按钮)")
	public ModelAndView opDialogBtn(HttpServletRequest request, HttpServletResponse response) throws Exception {
		String isRequired = RequestUtil.getString(request, "isRequired");
		String actDefId = RequestUtil.getString(request, "actDefId");
		String taskId = RequestUtil.getString(request, "taskId");
		String reject = RequestUtil.getString(request, "reject", "0");
		...
		List<ForkTaskReject> forkTaskExecutor = bpmActService.forkTaskExecutor(taskId);
		// 获取常用语
		List<String> taskAppItems = taskAppItemService.getApprovalByDefKeyAndTypeIdBtn(bpmDefinition.getDefKey(), bpmDefinition.getTypeId(), type);
		return getAutoView().addObject("isRequired", isRequired)
							.addObject("reject",reject)
						    .addObject("taskAppItems", taskAppItems)
						    .addObject("forkTaskExecutor", forkTaskExecutor);
	 ...
	 * @throws Exception 
	 */
	@RequestMapping("isTaskEndOpDialog")
	@Action(description = "填写意见(结束)")
	public ModelAndView isTaskEndOpDialog(HttpServletRequest request, HttpServletResponse response) throws Exception {
		String actDefId = RequestUtil.getString(request, "actDefId");
		String type = RequestUtil.getString(request, "type");
		...
		return getAutoView().addObject("taskAppItems", taskAppItems).addObject("message", message);
	}
