...
public class ProcessServiceImpl implements ProcessService {
	...
	protected static Logger logger = LoggerFactory.getLogger(ProcessServiceImpl.class);
	...
	public String getProcessRun(String xml) {
		...
		if (StringUtils.isNotEmpty(defKey)) {
			...
			if (BeanUtils.isNotEmpty(bpmDefinition)) {
				...
				return genMessage(false, "没有找到对应的流程定义！");
			}
		}
	...
	public String start(String xml) {
		...
		try {
	    	...
	    	Object pwd = null;
	    	if(data != null) {
				pwd = data.getMainField("F_CXMM");
			}
			Element element = ProcessRun2Xml(null, processRun);
			if(null != pwd) {
				Dom4jUtil.addAttribute(element, "cxmm", pwd.toString());
			}
			return Dom4jUtil.docToPrettyString(element.getDocument());
		} catch (Exception e) {
			e.printStackTrace();
	...
	private Element ProcessRun2Xml(Element parentElement, ProcessRun processRun) throws Exception {
		...
		Dom4jUtil.addAttribute(docRoot, "globalFlowNo", processRun.getGlobalFlowNo());
		return docRoot;
	}
	...
	public Long[] getLongAryByStr(String str)
/*      */   {
...
/*      */   }
	...
	@Override
	public String delMessageReceiver(String id) {
		try{
			Long[] lAryId =getLongAryByStr(id);
			receiverService.delByIdsCascade(lAryId);
		}
		catch(Exception ex){
			logger.error("delMessageReceiver error:", ex);
			return "{\"error_code\": 1,\"message\": \"删除失败!\"}";
		}
		return "{\"error_code\": 0,\"message\": \"操作成功!\"}";
	}

	@Override
	public String markMessageReceiver(String id,String account) {
		Long[] lAryId =getLongAryByStr(id);
		try{
//			receiverService.updateReadStatus(lAryId);
			SysUser sysUser = sysUserService.getByAccount(account);
			receiverService.updateReadStatusforWebservice(lAryId,sysUser);
		}
		catch(Exception ex){
			logger.error("markMessageReceiver error:", ex);
			return "{\"error_code\": 1,\"message\": \"操作失败!\"}";
	...
	@Override
	public String delMessageSend(String id) {
		try{
			Long[] lAryId =getLongAryByStr(id);
			sendService.delByIds(lAryId);
		}
		catch(Exception ex){
			logger.error("delMessageSend error:", ex);
			return "{\"error_code\": 1,\"message\": \"操作失败!\"}";
		}
		return "{\"error_code\": 0,\"message\": \"操作成功!\"}";
	}
	...
	@Override
	public String checkRecover(String id, String account) {
		String res = "";
		try{
			SysUser sysUser = sysUserService.getByAccount(account);
			if(sysUser != null && !"".equals(sysUser)){
				ResultMessage result = processRunService.checkRecover(Long.parseLong(id), sysUser.getUserId());
				res = "{\"error_code\": "+result.getResult()+",\"message\": \""+result.getMessage()+"\"}";
			}else{
				res = "{\"error_code\": 0,\"message\": \"操作失败!\"}";
			}
		}catch(Exception ex){
			logger.error("checkRecover error:", ex);
			res = "{\"error_code\": 0,\"message\": \"操作失败!\"}";
		}
		return res;
	}
	...
	//测试接口
	@Override
	public SysUser webServiceDemo(String account) {
		SysUser sysUser=sysUserService.getByAccount(account);
		return sysUser;
	}
