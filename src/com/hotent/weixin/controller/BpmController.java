...
public class BpmController extends GenericController {
	...
	public List<BpmDefinition> getMyDef(HttpServletRequest request, HttpServletResponse response){
		...
		//typeId = sysUserDao.getYhlxByUserId(userId);
	...
	public JSONObject getStartForm(HttpServletRequest request, HttpServletResponse response) {
		...
		try{
			...
			if(StringUtil.isEmpty(pk)){
				...
				data=iWeixinFormService.getDraftData(formTable, pk, false);
			}
			//计算返回数据。
	...
	public JSONObject getTaskForm(HttpServletRequest request, HttpServletResponse response) {
		...
		try{
			...
			//获取是否进行意见填写
			boolean isHideOption = false;
			if(bpmNodeSet.getIsHideOption()==1){
				isHideOption=true;
			}
			...
			flowParam.put("isHideOption",isHideOption);
	...
	@RequestMapping("getCopytoTransJson")
	@Action(description = "查看抄送转发给我的流程")
	@ResponseBody
	public ListModel getCopytoTransJson(HttpServletRequest request, HttpServletResponse response){
		QueryFilter filter = new QueryFilter(request, true);
	...
	@RequestMapping("updReaded")
	@Action(description = "更新转发抄送实例已读状态")
	public void updReaded(HttpServletRequest request, HttpServletResponse response) throws Exception{
		Long copyId = RequestUtil.getLong(request,"copyId",0L);
		try {
			if(copyId!=null && copyId>0){
				bpmProCopytoService.markCopyStatus(copyId.toString());
			}
		} catch (Exception e) {
