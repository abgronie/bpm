		...
		if(sysUser==null){
			...
			if(StringUtil.isEmpty(code)){
				...
				try{
					...
					SysUser user = (SysUser) ContextUtil.getCurrentUser();
					if(user == null){
						String rdUrl= PropertyUtil.getByAlias(SysPropertyConstants.PLATFORM_URL) +"/proxy?redirect="+PropertyUtil.getByAlias(SysPropertyConstants.PLATFORM_URL)+"/weixin/proxy.html?errorText=2";
						resp.sendRedirect(rdUrl);
					}else{
						if(user.getHasSyncToWx() ==1){
							user.setHasSyncToWx(2);
