...
public class WxUserService implements IUserService{
	...
	public void create(SysUser sysUser){
		...
		if(StringUtils.isEmpty(sUser.getWeixinid())) 
			throw new RuntimeException(sUser.getFullname()+"添加微信通讯录失败 ：没有填写微信信息 ");
		
		if(sUser.getHasSyncToWx() ==1 || this.queryUser(sysUser)==true ) {
			this.update(sysUser);
...
//			this.invite(user.getUserid());
	...
	public void update(SysUser sysUser){
		...
		if(this.queryUser(sysUser)==false) 
			throw new RuntimeException(user.getName()+"更新微信通讯录失败 ： 该用户不存在企业微信通讯录");
		//验证存在，即可更新
...
//			if(u.getHasSyncToWx()==1) this.invite(user.getUserid());
	...
	public boolean queryUser(SysUser sysUser){
		WxUser user = OrgConvertUtil.sysUserToWxUser(sysUser);
		//获取企业微信用户，判断是否存在
		String getUserUrl=WeixinConsts.getUserUrl()+sysUser.getAccount();
