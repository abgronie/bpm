...
PictureShowPlugin={
		...
		init:function(){
			...
			this.pictureWidth = 160;         //可以展示控件 上面大图片的宽  默认			
			...
			for ( var i = 0; i < this.fileObjs.length; i++) {
				...
				var jsonData;
				try{
					jsonData = PictureShowPlugin.getData(name);    //获取隐藏域的JSON数据
				}catch(ex){
					console.debug(ex);
					continue;
				}
		...
		upLoadPictureFile:function(t){
			...
			/*if(this.isZoomNum){   //第一次做上传修改时要ZOOM窗口序号化
				...
				if("m:xscbsqb:zp" == fieldName){
					pictureShowUploadDialog2({uploadType:"pictureShow",fileFormates:"*.jpg;*.jpeg;*.bmp;*.png;*.gif;*.tiff;*.pcx;*.fpx;*.svg;*.psd",callback:function (fileIds,fileNames,filePaths,extPaths){				
						//没有返回上传的图片相关信息时禁止执行
...
function pictureShowUploadDialog2(conf) {
	if(!conf) conf={};
	var url=__ctx + "/platform/system/sysFile/uploadDialog.ht";
	var winArgs="dialogWidth:450px;dialogHeight:300px;help:0;status:0;scroll:1;center:1";
	url=url.getNewUrl();
			...
			if(conf.callback){
				...
				var extPath = new Array();
		 		for(var i=0;i<rtn.length;i++){
		 			...
		 			console.log("=====================检测图片开始=======================");
					var note = fileObj.note;
					var size = note.split(".")[0];
					console.log("=====================检测图片结束=======================");
					if(size > 100){
						$.ligerDialog.error('上传的图片不符合要求，请重新上传！');
						return false;
					}
