...
<script type="text/javascript" src="bianhaoDialogController.js"></script>
...
<script type="text/javascript">
	...
	$(function() {
		...
		initName();
		scope.external.name = "bianhao";
		scope.external.comment = "编号";
		scope.external.dbType.length = "50";
		scope.external.buttoncontent = "编号";
		...
		dialog.onok = function() {
			...
			var span;
			if (!targetEl) {//新增模式
				span = $("<span class='personpicker' parser='bianhaoedit'></span>");
			...
			span.append("<input type='text' /><a href='javascript:;' class='link bianhao'>" + scope.external.buttoncontent + "</a>");
	...
	<form id="frmSubmit">
		<table class="edit_table">
			...
			<tr>
					<th>字段注释:</th>
					<td>
						<input ng-model="external.comment" type="text" validate="{required:true}" readonly>
					</td>
					<th>字段名称:</th>
					<td>
						<input ng-model="external.name" ht-pinyin="external.comment" type="text" validate="{required:true,varirule:true}" readonly>
					</td>
				</tr>
				...
				<tr>
					<th>数据类型:</th>
					<td>
						<select ng-model="external.dbType.type" readonly>
					...
					<td class="dbformat_td" ng-if="external.dbType.type=='varchar'">
						长度:
						<input ng-model="external.dbType.length" style="width: 60px;" type="text" readonly>
					</td>
				</tr>
				<tr>
					<th>选项:</th>
				...
				<tr class="valuefrom0" ng-if="external.valueFrom.value=='0'">
					...
					<td colspan="3">
						<select ng-model="external.valueFrom.content" ng-options="m.name as m.name for m in ruleList">
							<option value="">年份日期加序号</option>
						</select>
						<!-- 
						<select ng-model="external.valueFrom.content">
							<option value="yyyymmdd">年份日期加序号</option>
							<option value="yyyy">年份加序号</option>
						</select>
						 -->
					</td>
				</tr>
