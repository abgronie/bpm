...
DataRightsApp.service('dataRightsService', ['$http','$jsonToFormData','BaseService', function($http,$jsonToFormData,BaseService) {
	var service = {
		init:function($scope){
			...
			$scope.formViewMainFields = formViewMainFields;
						...
						'<table id="displayFieldTbl"  class="table-grid">'+
							...
							'<tbody>'+
								'<tr var="displayFieldTr" ng-repeat="f in displayFields track by $index">'+
									...
									'</td>'+
									'<td>'+
										'<tool-buttons list="displayFields" index="{{$index}}" type="5"></tool-buttons>'+
									'</td>'+
								'</tr>'+
							'</tbody>'+
						'</table>'+
						...
						'<table id="manageTbl"  class="table-grid">'+
							...
							'<tbody>'+
								'<tr var="manageTr" ng-repeat="f in manageFields">'+
									...
									'</td>'+
									'<td ng-if="!service.noRights">'+
										'<div style="display: inline;" right="f.right[0]" choose-target></div>'+
										'<div ng-if="f.name == \'import\'" style="display: inline;">'+
											'&nbsp&nbsp唯一标识:<select ng-model = "f.importIdField" class="ht-input w-120">'+
											'<option value="">请选择</option>'+
											'<option ng-repeat="f in bpmFormTable.fieldList">{{f.fieldDesc}}</option>'+
											'</select>'+
										'</div>'+
									'</td>'+
									'<td>'+
							...
							'<div >'+
								'<div class="condition-cols-div">'+
									'<table id="condition-columnsTbl" cellpadding="0" cellspacing="0" border="0" class="table-detail">'+
										...
										'</thead>'+
										'<tbody>'+
											'<tr ng-repeat="field in formViewMainFields" ng-if="field.isVirtual!=1" ng-class="{odd:$index%2==0,even:$index%2!=0,\'tr-select\':field.checked}" ng-click="field.checked=!field.checked">'+
						...
						'<div class="sort-cols">'+
							'<div class="sort-cols-div">'+
								'<table id="sort-columnsTbl" cellpadding="0" cellspacing="0" border="0" class="table-detail">'+
									...
									'</thead>'+
									'<tbody>'+
										'<tr  ng-repeat="field in formViewMainFields" ng-class="{odd:$index%2==0,even:$index%2!=0}" ng-click="field.checked=!field.checked">'+
											'<td>'+
