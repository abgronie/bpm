...
function changeScrollHeight(noNeedFoldHeight,time){
	...
	window.setTimeout(function(){
		...
		var tableBodyHeight = 500;
		if(defaultMenuHeight != 0){
			tableBodyHeight = $(window).height()-tableBody[0].offsetTop-defaultMenuHeight-foldHeight-moreHeight-51;
			// 设置蒙版高度
			...
			tableBodyHeight = $(window).height()-tableBody[0].offsetTop-extendIndexMenuHeight-foldHeight-moreHeight-51;
			// 设置蒙版高度
		...
		window.setTimeout(function(){
			var $table = $('.panel-table table');
			if($table.floatThead && $($table).find("thead").length ){
				$table.floatThead({
					scrollContainer: function($table){
						return $table.closest('.panel-table');
					}
				});
			}
			tableBody.height(tableBodyHeight);
		});
