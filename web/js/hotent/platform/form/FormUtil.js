...
FormUtil.pushDataToForm = function(data,fields,isToLowerCase,isAddRow,me,isDelSubTableBeforInsert){
	...
	for(var i=0;i<fields.length;i++){
		...
		for(var j=0,target;target=targets[j++];){
			...
			var tableName=target.split("-")[0];
			var targetFileName = target.replace("-",":");//把筛选條件由根据列名匹配替换成。表名+列名匹配。防止子表主表相同列名匹配錯誤
		...
		CommonDialog(name,function(data){
			...
			if(window.checkDataBeforeInsert){
				if(fields.length >0 && data){
					...
					if(targets && targets.length>0)  
						if(!checkDataBeforeInsert(data,targets[0].split("-")[0])) return;	
				}
			}
 ...
 * 获取关联的需要变动的字段
 */
FormUtil.getTargetObj = function(filter,$tab){
	if(!$changeObj){
		return $("[name='"+filter+"']",$tab);
	}
	var formtype = $changeObj.parents("[formtype]"),
		me = null;
	if(formtype&&formtype.length>0){
		me = $("[name='"+filter+"']",formtype,$tab);
	}
	else{
		me = $("[name='"+filter+"']",$tab);
	}
 ...
 * @param {} changeField
 */
FormUtil.doChangeField = function(choiceKey,c,$tab){
	var me = FormUtil.getTargetObj(c.key,$tab);
			...
			changeObj.die("change").live("change",function(e){
				...
				if($changeFunc.length>0){
					for(var y=0,o;o=$changeFunc[y++];){
						FormUtil.doChange(o.choiceKey,o.changes,$tab);
					}
				}
			});
...
FormUtil.initCommonQuery=function(){
	$("[query]").live("keyup change", function(event){
		...
		DoQuery(queryCond,function(data){
			...
			var tabName=obj.attr('name').split(':')[1];
			for(var i=0;i<fields.length;i++){
				var json=fields[i];
				var src=json.src;
				var targets=json.target;
				...
				while(target=targets.pop()){ 
					if(!target)return;
					var filter="[name$=':"+tabName+':'+target+"']";
