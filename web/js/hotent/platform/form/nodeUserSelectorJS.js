...
function  startOrPrevDialog(conf){
	var type = "";
	var userType = "";
	if(conf.cmpIds != null && conf.cmpIds != ""){
		var cmpIdsJson = $.parseJSON(conf.cmpIds);
		type = cmpIdsJson.type;
		userType = cmpIdsJson.userType;
	}
	url=__ctx + "/platform/bpm/bpmNodeUser/startOrPrevDialog.ht?nodeUserId=" +conf.nodeUserId +"&defId=" +conf.defId +"&type=" +type +"&userType=" +userType;
	url=url.getNewUrl();
	var that = this;
	$.ligerDialog.open({
		height:500,
