...
function lastAjax(obj,tableIdCode){
	var lastPage=parseInt($("#totalPage"+tableIdCode).val().replace(/,/g,''));
	if(lastPage<=0) return;
	goPageAjax(obj,lastPage,tableIdCode);
}

function previousAjax(obj,tableIdCode){
	var currentPage=parseInt($("#currentPage"+tableIdCode).val().replace(/,/g,''));
	currentPage-=1;
...
function nextAjax(obj,tableIdCode){
	var currentPage=parseInt($("#currentPage"+tableIdCode).val().replace(/,/g,''));
	var totalPage=parseInt($("#totalPage"+tableIdCode).val().replace(/,/g,''));
...
function openLinkDialog(conf){
	...
	var isIfram = conf.isIfram||false;
