...
<#macro genToolBar manage managePermission actionUrl>
	...
	<#elseif manage.name == 'import' >
		<#if managePermission.import>
			<div class="group"> <a class="link import" action="${actionUrl.import}?importIdField=${manage.importIdField}"  onclick="openLinkDialog({scope:this,width:450,height:400,title:'导入'})"   href="javascript:;"><span></span>${manage.desc}</a></div>
			<div class="l-bar-separator"></div>
		</#if>
	<#--批量删除-->
	<#elseif manage.name == 'del' >
		<#if managePermission.del>
			<div class="group"><a class="link del" action="${actionUrl.del}">${manage.desc}</a></div>
			<div class="l-bar-separator"></div>
		</#if>
	<#--打印-->	
...
<#--表体-->				
				<#noparse>
					...
					<tr class="<#if data_index % 2 == 0>odd</#if><#if data_index % 2 == 1>even</#if>">
					    ...
					    <td style="width:30px;">
                      	  <input class="pk" type="checkbox" value="<#noparse>${data.</#noparse>${pkField}<#noparse>}</#noparse>" name="__pk__">
                        </td>
               	<#noparse>
