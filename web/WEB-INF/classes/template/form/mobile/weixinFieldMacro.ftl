...
<#macro input field type tableName >
		<#if field.fieldType == "varchar"><#---字符串类型-->
			<#switch field.controlType>
				<#case 1><#--单行文本框-->
				     <#if field.identity== ''>
				      <input ht-input="${getFieldName(field,type)}" name="${getFieldName(field,type)}" ng-model="${getFieldName(field,type)}" ht-validate="${getValidRule(field)}" permission="${getPermission(field,type,tableName)}" placeholder="${field.fieldDesc}" class="am-form-field" />
                     <#else>
                     <input ht-input="${getFieldName(field,type)}" name="${getFieldName(field,type)}"  ht-identity="${field.identity}"  ng-model="${getFieldName(field,type)}" ht-validate="${getValidRule(field)}" permission="${getPermission(field,type,tableName)}" placeholder="${field.fieldDesc}" class="am-form-field" />
			         </#if>
				<#break>
				<#case 2><#--多行文本框-->
				...
				<#break>
				<#case 11><#--下拉选项-->
					<select ht-select="${getFieldName(field,type)}" name="${getFieldName(field,type)}" data-am-selected="{btnStyle:'success'}" ng-model="${getFieldName(field,type)}" ht-validate="${getValidRule(field)}" permission="${getPermission(field,type,tableName)}" class="am-form-field">
				...
				<#break>
				<#case 16><#--隐藏域-->
					<input ht-input="${getFieldName(field,type)}" name="${getFieldName(field,type)}" ng-model="${getFieldName(field,type)}" permission="${getPermission(field,type,tableName)}" placeholder="${field.fieldDesc}" class="am-form-field" type="hidden" />
				<#break>
				<#case 17><#--角色选择器(单选)-->
               	<#break>
				<#case 18><#---组织选择器(单选)-->
		...
		<#elseif field.fieldType == "number"><#---数字类型-->
			...
			<#else><#--否则数字输入-->
				 <input ht-formator="${toSingleQuotes(field.ctlProperty)}" ht-input="${getFieldName(field,type)}" ng-model="${getFieldName(field,type)}" ht-validate="${getValidRule(field)}" permission="${getPermission(field,type,tableName)}" placeholder="${field.fieldDesc}" class="am-form-field" />
			</#if>
