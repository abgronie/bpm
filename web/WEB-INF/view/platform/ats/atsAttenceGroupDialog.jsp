...
<script type="text/javascript">
	...
	function add(data) {
		...
		var groupId = aryTmp[0];
		var len = $("#attenceGroup_" + groupId).length;
		if (len > 0) return;
		var attenceGroupTemplate = $("#attenceGroupTemplate").val();
		var html = attenceGroupTemplate.replace("#groupId", groupId)
			.replace("#data", data)
			.replace("#name", aryTmp[1]);
		$("#attenceGroupList").append(html);
	};
	function del(obj) {
	...
	function selectData(){
		...
		if(isSingle=='true'){
			...
			chIds = $("input[name='group']", $("#attenceGroupList"));
		}
		...
		var obj={id:ids.join(","),name:names.join(","),code:codes.join(",")};
	...
	<div id="listLayout" style="height:100%;">
		<div position="center">
				<iframe id="listFrame" name="listFrame" height="95%" width="100%" frameborder="0"
					src="${ctx}/platform/ats/atsAttenceGroup/selector.ht?isSingle=${isSingle}"
				 ></iframe>
		</div>
		<c:if test="${param.isSingle==false}">
			   <div position="right" title="<span><a onclick='javascript:dellAll();' class='link del'>清空 </a><input type='text' class='quick-find' title='快速查询'/></span>" style="overflow: auto;height:95%;width:170px;">
          			<table width="145"   class="table-grid table-list"  cellpadding="1" cellspacing="1">
          				<tbody id="attenceGroupList">
          					<tr class="hidden"></tr>
          				</tbody>
					</table>
			  </div>
		</c:if>
		<textarea style="display: none;" id="attenceGroupTemplate">
			<tr id="attenceGroup_#groupId">
		  			<td>
		  				<input type="hidden" name="group" value="#data"><span>#name</span>
		  			</td>
