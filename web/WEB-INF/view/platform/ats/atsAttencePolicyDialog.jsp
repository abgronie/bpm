...
<script type="text/javascript">
	...
	function add(data) {
		...
		var len = $("#attencePolicy_" + policyId).length;
		if (len > 0) return;
		var attencePolicyTemplate = $("#attencePolicyTemplate").val();
		var html = attencePolicyTemplate.replace("#policyId", policyId)
			.replace("#data", data)
			.replace("#name", aryTmp[1]);
		$("#attencePolicyList").append(html);
	};
	function del(obj) {
	...
	function selectData(){
		...
		if(isSingle=='true'){
			...
			chIds = $("input[name='policy']", $("#attencePolicyList"));
		}
		var ids =[],
		 	names =[];
	...
	<div id="listLayout" style="height:100%;">
		<div position="center">
				<iframe id="listFrame" name="listFrame" height="95%" width="100%" frameborder="0"
					src="${ctx}/platform/ats/atsAttencePolicy/selector.ht?isSingle=${isSingle}"
				 ></iframe>
		</div>
		<c:if test="${param.isSingle==false}">
			   <div position="right" title="<span><a onclick='javascript:dellAll();' class='link del'>清空 </a><input type='text' class='quick-find' title='快速查询'/></span>" style="overflow: auto;height:95%;width:170px;">
          			<table width="145"   class="table-grid table-list"  cellpadding="1" cellspacing="1">
          				<tbody id="attencePolicyList">
          					<tr class="hidden"></tr>
          				</tbody>
					</table>
			  </div>
			  <textarea style="display: none;" id="attencePolicyTemplate">
				<tr id="attencePolicy_#policyId">
			  			<td>
