...
<script type="text/javascript">
	...
	$(function(){
		 $("#atsHolidayPolicyItem>tbody").find("tr").bind('click', function() {
			 if(isSingle!='true'){
		 ...
		 $("#chkall").bind("click",function(){
				...
				var checkboxs=$(":checkbox",$("#atsHolidayPolicyItem>tbody"));
...
<body>
	...
	<div  class="panel-top">
		<div class="panel-search">
			<form id="searchForm" method="post" action="${ctx}/platform/ats/atsHolidayPolicy/selector.ht" >
				<ul class="row">
	    ...
	    <display:table name="atsHolidayPolicyList" id="atsHolidayPolicyItem" requestURI="selector.ht" sort="external" cellpadding="1" cellspacing="1" class="table-grid">
			<display:column title="${checkAll}" media="html" style="width:30px;">
				<c:choose>
					<c:when test="${isSingle==false}">
						<input type="checkbox" class="pk" name="data" value="${atsHolidayPolicyItem.id}#${atsHolidayPolicyItem.name}">
						
					</c:when>
					<c:otherwise>
						<input type="radio" class="pk" name="data" value="${atsHolidayPolicyItem.id}#${atsHolidayPolicyItem.name}">
					</c:otherwise>
				</c:choose>
			</display:column>
		...
		<hotent:paging tableId="atsHolidayPolicyItem" showExplain="false"/>
</body>
</html>
