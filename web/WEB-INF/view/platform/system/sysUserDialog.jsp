...
<title>选择用户 </title>
	...
	<script type="text/javascript">
		...
		var ifShowChildNode=${ifShowChildNode};
		var rol_Tree=null; 
		var org_Tree=null; 
		var pos_Tree=null; 
		var onl_Tree=null; 
		...
		function load_Org_Tree(){
			...
			var setting = {
					...
					callback:{
						...
						onNodeCreated: orgTreeOnNodeCreated
					}
		...
		//节点创建完成后，如果是只选部门或只选上级，改变图标
		function orgTreeOnNodeCreated(event, treeId, treeNode, msg){
			if(!ifShowChildNode){
		...
		function load_Rol_Tree(){
			...
			function selectUserName(){
				...
				var url="${ctx}/platform/system/sysUser/selector.ht?searchBy=5&userName="+userName;
				$("#userListFrame").attr("src",url);
			}
			
			function selectUserAccount(){
				var account = $("input[name='account']").val();
				var url="${ctx}/platform/system/sysUser/selector.ht?searchBy=5&account="+account;
				$("#userListFrame").attr("src",url);
			}
...
<body>
	<div id="defLayout" style="height:100%;">
		<div id="leftMemu" position="left" title="查询条件<input type='checkbox' id='includSub'/>包含子分类" style="overflow: auto; float:left;width: 100%; height:95%">
			<!-- 按工号查找 by byq 
			<div title="按工号查找" style="overflow: hidden;">
				<table border="0" width="100%" class="table-detail">
					<tr>
						<td>
							<span class="label" style="width:60px;">工号:</span>
						</td>
						<td>
							<input id="account" name="account" type="text" size="10">
						</td>
						<td>
							<a class="link detail" href="javascript:selectUserAccount();">&ensp;</a>
						</td>
					</tr>
				</table>
			</div>
			-->
			<!-- 按姓名查找 
			<div title="按姓名查找" style="overflow: hidden;">
				<table border="0" width="100%" class="table-detail">
					<tr>
						<td>
							<span class="label" style="width:60px;">姓名:</span>
						</td>
						<td>
							<input id="userName" name="userName" type="text" size="10">
						</td>
						<td>
							<a class="link detail" href="javascript:selectUserName();">&ensp;</a>
						</td>
					</tr>
				</table>
			</div>
			-->
			<c:if test="${isLeader==false and isNext==false}">
	...
	<div position="bottom"  class="bottom" style="margin-top:3px;">
		<a href="javascript:;" class="button"  onclick="selectUser()" style="margin-right:10px;" ><span class="icon ok"></span><span >选择</span></a>
		<a href="javascript:;" class="button"  onclick="clearUser()"><span class="icon clean" ></span><span class="chosen" >清空</span></a>
