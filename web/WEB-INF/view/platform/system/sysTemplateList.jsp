				...
				<form id="searchForm" method="post" action="list.ht">
					...
					<li>
						...
						<select name="Q_useType_L">
							...
							<option value="30"<c:if test="${param['Q_useType_L'] eq 30}"> selected="selected"</c:if>>创建人消息提醒</option>
							<option value="31"<c:if test="${param['Q_useType_L'] eq 31}"> selected="selected"</c:if>>通知发起人</option>
						</select>
					</li>
					</ul>
				</form>
			...
			<display:table name="sysTemplateList" id="sysTemplateItem"
				...
				<display:column titleKey="模版用途">
					<c:choose>
						...
						<c:when test="${sysTemplateItem.useType eq 30}">创建人消息提醒</c:when>
						<c:when test="${sysTemplateItem.useType eq 31}">通知发起人</c:when>
