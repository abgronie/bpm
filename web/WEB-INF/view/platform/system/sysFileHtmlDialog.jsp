...
<%
	String maxUploadSize2 = AppConfigUtil.get("maxUploadSize");
	String type2 = AppConfigUtil.get("type");
%>
...
<c:set var="max" value="${param.max}"></c:set>
<c:set var="maxUploadSize" value="${param.maxUploadSize}"></c:set>
...
<script type="text/javascript">
	var maxUploadSize2 = '<%=maxUploadSize2%>';
	var type2 = '<%=type2%>';;
	var	type= 'jpg,png,bmp,jpeg,pdf,doc,docx,xls,xlsx,zip,rar';
	var	max = Number('${max}');
	var	maxUploadSize = Number('${maxUploadSize}');
	function getData(){
	...
	app.controller('uploadCtrl',[ '$scope', 'FileUploader', function($scope, FileUploader) {
			...
			if(type2){
				...
				uploader.filters.push({
					...
					fn : function(item, options) {
						...
						var result = true;
						var extName = item.name.split(".")[item.name.split(".").length-1]
						if(type2.indexOf(extName.toLowerCase()) < 0){
							result = false;
							alert("文件类型只能是"+type2);
		                }
			...
			//if(maxUploadSize2&&typeof maxUploadSize2=='number'){
			if(maxUploadSize2 != null && maxUploadSize2 != ""){
				//by byq上传文件的大小过滤器 
				uploader.filters.push({
					name : 'sizeFilter',
					fn : function(item, options) {
						var result = item.size <= maxUploadSize2;
						if(!result){
							alert("单个文件大小不能超过"+maxUploadSize2/1024/1024+"M");
						}
						//!result&&($.ligerDialog.alertExt("提示信息","单个文件大小不能超过"+maxUploadSize2/1024/1024+"M"));
						return result;
					}
				});	
			}
