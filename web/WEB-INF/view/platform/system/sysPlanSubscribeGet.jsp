	...
	<div class="panel">
		<div class="panel-top">
			<div class="tbar-title">
				<span class="tbar-label">日程信息 --订阅</span>
			</div>
			<div class="panel-toolbar">
				<c:choose>
					<c:when test="${type == 'subscribe'}">
						<c:choose>
							<c:when test="${sysPlan.subscribeId == null||sysPlan.subscribeId == ''||sysPlan.subscribeId == '0'}">
								<div class="group">
									<a class="link edit"  id="subscribeSysPlan" onclick="subscribeSysPlan('${sysPlan.id}');"  href="#" ><span></span>订阅</a>
								</div>
							</c:when>
							<c:otherwise>
								<div class="group">
									<a class="link reset" id="cancelSysPlan" onclick="cancelSysPlan('${sysPlan.subscribeId}');" href="#" ><span></span>退订</a>
								</div>
							</c:otherwise>
						</c:choose> 
						...
						<div class="toolBar">
							<div class="group">
								<a class="link back" href="${ctx}/platform/system/sysPlan/underList.ht?queryUserId=${queryUserId}"><span></span>返回</a>
							</div>
						</div>
					</c:when>
					<c:otherwise>
						<div class="toolBar">
							<div class="group">
								<a class="link back" href="${ctx}/platform/system/sysPlan/subscribeList.ht"><span></span>返回</a>
							</div>
						</div>
					</c:otherwise>
				</c:choose> 
