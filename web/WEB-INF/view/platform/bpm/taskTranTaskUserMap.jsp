					...
					<c:forEach items="${nodeTranUser.nodeUserMapSet}" var="nodeUserMap">
						<div>
							...
							<c:forEach items="${nodeUserMap.taskExecutors}" var="executor">
								<!-- @byq 跳转人员默认选中 -->
								<input type="checkbox" checked="checked" include='1' name="${nodeUserMap.nodeId}_userId"  value="${executor.type}^${executor.executeId}^${executor.executor}"/>&nbsp;${executor.executor}
							</c:forEach>
							</span>
							&nbsp;&nbsp;<a href="javascript:;" class="link grant" onclick="selExeUsers(this,'${nodeUserMap.nodeId}','${nodeUserMap.nodeScope}')">选择...</a>
						</div>
					</c:forEach>
				</td> 
			</tr>
