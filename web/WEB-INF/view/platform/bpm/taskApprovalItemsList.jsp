		    ...
		    <display:table name="taskApprovalItemsList" id="taskApprovalItems" requestURI="list.ht" sort="external" cellpadding="1" cellspacing="1"  class="table-grid">
				...
				<display:column titleKey="作用范围" sortable="true" sortName="type">
					<c:choose>
						...
						<c:when test="${taskApprovalItems.type==8}">同意按钮常用语</c:when>
						<c:when test="${taskApprovalItems.type==9}">终止按钮常用语</c:when>
						<c:when test="${taskApprovalItems.type==10}">驳回按钮常用语</c:when>
					</c:choose>
				</display:column>
				...
				<display:column  titleKey="作用对象" sortable="true" >
					<c:choose>
						<c:when test="${taskApprovalItems.type==1 || taskApprovalItems.type==8 || taskApprovalItems.type==9 || taskApprovalItems.type==10}">所有流程</c:when>
