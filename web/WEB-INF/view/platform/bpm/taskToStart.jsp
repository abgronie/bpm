...
<html>
	<head>
		...
		function completeTask(){
			...
			//判断每条执行路径是否已选择用户
			//@byq 自由跳转时只需选中的增加人员即可（原先是两条必须都选择人员）
			var flag = false;
			var spanSelcUser = $("span[name='spanSelectUser'],#jumpUserDiv");
			var spanSelcUserLen = spanSelcUser.length;
			
			if(spanSelcUserLen > 1){
				var destTask = $("input[name='destTask']:checked").val();
				console.log(destTask);
				
				var destTaskUserId = $("input[name='"+destTask+"_userId']:checked").val();
				if(destTaskUserId != null && destTaskUserId != ""){
					flag = true;
				}
			}else{
				var tag = 0;
		...
		function opintionDialogBtn(reject, type){
			var isRequired = '${bpmNodeSet.isRequired}';
			var actDefId = $("#actDefId").val();
			var url= __ctx + '/platform/bpm/task/opDialogBtn.ht?taskId=${task.id}&isRequired=' +isRequired+'&actDefId='+actDefId + '&reject=' + reject + '&type=' + type;
			var voteContent = $("#voteContent").val();
			DialogUtil.open({
				height:350,
				width: 500,
		...
		function isTaskEndOpDialog(callBack){
			...
			var voteContent = $("#voteContent").val();
			var type = 9;
			var url="${ctx}/platform/bpm/task/isTaskEndOpDialog.ht?taskId=${task.id}&actDefId="+actDefId+"&type="+type;
		...
		function generatePDF(runId){
			$.get("${pageContext.request.contextPath}/platform/bpm/processRun/generatePDF.ht?runId="+runId,function(data,status){
		...
		function addSigns(runId){
			//$("#fjscTr").hide();
			$.get("${pageContext.request.contextPath}/platform/bpm/processRun/addSigns.ht?runId="+runId,function(data,status){
				console.log("Data: " + data + "\nStatus: " + status);
	...
	<form id="frmWorkFlow"  method="post" >
		 <div class="panel">
				...
				<div class="panel-body">
							...
							<c:choose>
								...
								<c:otherwise>
									<c:if test="${hasGlobalFlowNo }">
										<div align="right">申报号：${processRun.globalFlowNo}</div>
									</c:if>
									<c:choose>
		            ...
		            <input type="hidden" include="1" id=isCanBack name="isCanBack" value="${isCanBack}" /> 
		            <input type="hidden" include="1" id=gatherNode name="gatherNode" value="${gatherNode}" /> 
				</div>
         </div>
