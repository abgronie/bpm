	...
	<div class="hide-panel">
		<div class="panel-top">
			...
			<div class="panel-search">
					<form id="searchForm" method="post" action="alreadyMattersByjlList.ht">
							<ul class="row">
							<input type="hidden" name="nodePath" value="${param['nodePath']}" title="流程分类节点路径"></input>
							<li>	
								<span class="label">部门:</span>
								<select name="startOrgId">
									<option value="">--请选择--</option>
									<c:forEach items="${sysOrgList}" var="sysOrg">
										<option value="${sysOrg.orgId}" <c:if test="${sysOrg.orgId eq param['startOrgId']}">selected="selected"</c:if>>${sysOrg.orgName}</option>
									</c:forEach>
								</select> 
							</li> 
							<li>
								<span class="label">事项名称</span><input  type="text" name="Q_subject_SUPL"  class="inputText" value="${param['Q_subject_SUPL']}" />						
							</li>
							<li>
							...
							<li>
								<span class="label">状态:</span><select name="Q_status_SN">
									...
									<option value="2" <c:if test="${param['Q_status_SN']==2}">selected</c:if>>已办结</option>			
								</select>
							</li>
							<%-- 是否有全局流水号 
							...
							<div class="row_date">
							<li>
								<span class="label">申请时间&nbsp;从:</span> <input  name="Q_begincreatetime_DL"  class="inputText datePicker" datetype="1"  value="${param['Q_begincreatetime_DL']}" />
							</li>
							<li>
		...
		<div class="panel-body">
			    ...
			    <display:table name="processRunList" id="processRunItem" requestURI="alreadyMattersByjlList.ht" sort="external" cellpadding="1" cellspacing="1" class="table-grid">
					...
					<display:column titleKey="申请人" sortable="true" sortName="creator" style="text-align:left">
						<a href="javascript:userDetail('${processRunItem.creatorId}');">${processRunItem.creator}</a>
					</display:column>
					<display:column  titleKey="申请时间" sortable="true" sortName="createtime" style="text-align:left">
