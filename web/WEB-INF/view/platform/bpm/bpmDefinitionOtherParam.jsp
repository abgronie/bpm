...
<head>
	...
	function saveParam(){
		...
		var chkPendingInformStart = getMsgTypeList("chkPendingInformStart");
		...
		var params={defId:defId,taskNameRule:taskNameRule,toFirstNode:toFirstNode,
				...
				skipSetting:skipSetting.join(','),chkPendingInformStart:chkPendingInformStart.join(',')};
...
<body> 
			...
			<table class="table-detail" cellpadding="0" cellspacing="0" border="0">
				...
				<tr>
					<th width="20%">查看待办时发送回执给发起人:</th>
					<td>
					    <c:forEach items="${handlersMap}" var="item">
		                   <input type="checkbox" name="chkPendingInformStart" value="${item.key }"  <c:if test="${fn:contains(bpmDefinition.chkPendingInformStart,item.key)}">checked="checked"</c:if> />
                            ${item.value.title }
		                </c:forEach>
					</td>	
				</tr>
