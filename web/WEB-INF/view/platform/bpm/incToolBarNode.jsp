		...
		<div class="toolBar">
			<c:choose>
				<c:when test="${ isManage==0 }">
						...
						<div class="group"><a class="link print" onclick="printForm(${processRun.runId})"><span></span>打印表单</a></div>
							...
							<c:forEach items="${mapButton.button }" var="btn"  varStatus="status">
								<c:choose>
									...
									<c:when test="${btn.operatortype==10 }">
										...
										<div class="l-bar-separator"></div>-->
										<div class="group"><a class="link print" onclick="printForm(${processRun.runId})"><span></span>${btn.btnname }</a></div>
										<div class="l-bar-separator"></div>
									</c:when>
