...
<script type="text/javascript">
	...
	fileInit = function(){//解析表单的html，如果设置了自定义查询.则取出其查询结果，并将其添加到对应字段的options里面
		var querys;
		var filters = "select[selectquery][selectqueryed!='selectqueryed']";
	    querys = $(filters);
		querys.each(function(){
			var me = $(this), 
				selectquery = me.attr("selectquery"),name=me.attr("name");
			if (!selectquery)
				return true;
			selectquery = selectquery.replaceAll("'", "\"");
			var queryJson = JSON2.parse(selectquery);
			var query = queryJson.query;
			selectQueryFildsArr.push({queryJson:queryJson,name:name});
		});
		getvalue(0);
	};	
	getvalue = function(z) {
		if(z<selectQueryFildsArr.length){
			queryJson,name
			var queryJson=selectQueryFildsArr[z].queryJson;
			var name=selectQueryFildsArr[z].name;
			var query = queryJson.query;
			var querydataStr = "{";
			...
			DoQuery(queryCond, function(data) {
				...
				for(var i=0;i<list.length;i++){
					arr.push({"key":list[i][queryKey],"value":list[i][queryValue],"isDefault":"0"});
				}
				selectQueryArr.push({fildName:name,opts:arr});
				z=z+1;
				getvalue(z);
			},false);
		}else{
		...
		$.post(url,{formKey:setId,filter:filter},function(t){
			...
			for(var i=0;i<size;i++){
				...
				for(var j=0,m;m=c.fields[j++];){
					var optionArr=m.jsonOptions;
					var key =m.type==1?m.fieldName:((c.type==1?"m:":"s:") +c.key+":"+m.fieldName);
					if(optionArr.length<1){
						for(var z=0;z<selectQueryArr.length;z++){
							if(key==selectQueryArr[z].fildName){
								optionArr=selectQueryArr[z].opts;
							}
						}
					}
					...
					html.push(m.fieldId);
					html.push('" ops="');
					html.push(JSON2.stringify(optionArr).jsonEscape());
					html.push('" fieldtype="');
					html.push(m.type);
