...
.directive('htInput', function() {
	return {
    	...
    	compile: function(element, attrs){
    		...
    		// 主表的隐藏 已经在模板中通过ng-if 处理 这里可以不用  子表未改， 暂时保留
    		if(permission=='n')	{ $(element).hide(); }
	...
	return {
    	...
    	template : '<div class="am-u-sm-12 am-u-md-12" ng-if="permission==\'w\' || permission==\'b\'">\
						<span class="am-u-sm-6 m-u-md-4" ng-repeat="file in files"> 	\
							<button style="margin: 4px" class="am-btn am-btn-secondary" ng-click="onClick(file.id)">${file.name}</button> \
    	...
    	link:function(scope,element,attrs){
    		...
    		if(permission=="w" || permission=="b"){
    			...
    			str='<div class="am-form-icon am-form-feedback">\
	    		     	<input id="" type="text" ' + (ngChange ? ('ng-change="' + attrs.ngChange + '"') : '') + ' ng-model="'+name+'" ht-validate="'+(attrs.htValidate?attrs.htValidate:"{}")+'"  permission="'+permission+'">	\
...
.directive('selectquery',['commonService', function(commonService) {
	 return {
		...
		link: function(scope ,element, attrs){
			...
			var isInit=true;
			if(arr.length<3){
				var val =scope.$parent.item[arr[1]]||'';
			}else{
				var val =scope.$parent.data.main[arr[2]]||'';
			}
			var toQuery = function(){
				...
				for (var i = 0,query;query=selectqueryCof.query[i++];){
					...
					if(query.trigger=='-1' && fieldValue == ""){//如果自定义查询的输入框无值，则用默认值
				...
				var condition = {
						...
						page : 0,
						pagesize : 0}
				 if(JSON.stringify(querydata) == "{}"){//如果没有查询条件
					 var fileArr=attrs.name.split('.');
					 var fileName='';
					 if(fileArr.length>2){
						 fileName=fileArr[2];
						 scope.$parent.data.main[fileName]='';
					 }else{
						 fileName=fileArr[1];
						 scope.$parent.item[fileName]='';
					 }
					 element.empty();
					return false;
				 }
				DoQuery(condition,function(data){
					...
					element.empty();
					var htmlStr='';
					if(!isInit){//不是初始化的时候。当上级触发本级下拉框重置的时候，则将该级的值置空，以便重置其子级选择框
						 var fileArr=attrs.name.split('.');
						 var fileName='';
						 if(fileArr.length>2){
							 fileName=fileArr[2];
						...
						htmlStr+="<option selected='selected' value='' >请选择</option>";
					}else{
						htmlStr+="<option value='' >请选择</option>";
					}
					for (var i = 0,item;item=data.list[i++];){
						var datavalue = item[selectqueryCof.binding.value];
						var datakey = item[selectqueryCof.binding.key];
						if(datakey==val && isInit){//如果是初始化的时候，并且该选项的值等于改字段的值，则默认选中该项
							isInit=false;
							htmlStr+="<option selected='selected' value='"+datakey+"'>"+datavalue+"</option>";	
						}else{
							htmlStr+="<option value='"+datakey+"'>"+datavalue+"</option>";	
						}
					}
					element.append(htmlStr);
					if(permission=='r'){//下拉框加载完毕之后判断，如果是只读，去除下拉框，把值填充进去
        	...
        	$scope.pushDataToForm=function(returnData,mappingConf){
        		...
        		for (var i = 0; i < returnData.length; i++) {
					...
					for (var int = 0,mapping;mapping= mappingConf[int++];){ 
						...
						var value =returnData[i][mapping.src] || returnData[i][mapping.src];
...
formDirective.directive('query',['$rootScope', function($rootScope) {
    return {
    	restrict : 'AE',
        link: function($scope, element, attrs) {
         	var permission=attrs.permission;
        	...
        	if(permission && permission !='w' && permission != 'b'){
      		...
      		var obj = $(element),
      		queryStr=obj.attr("query"),
      		queryArr = eval("("+queryStr+")"),
			evt = queryArr.evt,
			oldVal='';
        	...
        	$scope.doGangedQuery = function(){
			   ...
			   if(next&&next.length > 0){
				...
				var isSubTab = (element.parents("[type='subtable']").length >0);
			    if(!value){//如果查询条件无值。则清除之前填充的数据
					for(var i=0;i<fields.length;i++){
						var targets=fields[i].target;
						var target;
						while(target=targets.pop()){ 
							if(!target)return;
							var filter=isSubTab?("[ng-model='item."+target +"']") :("[ng-model='data.main."+target +"']");
...
formDirective.directive('htIdentity',['$rootScope', function($rootScope) {
    return {
    	restrict : 'AE',
        link: function($scope, element, attrs) {
				var alias = attrs.htIdentity;  
				$.get(__ctx + "/platform/system/identity/nextId.ht?alias="+alias,function(data){
						var mName=attrs.ngModel.split('.')[1];
						$scope.item[mName]=data;
						$(element).val(data);	
				})
        }
    }
...
function DoQuery(condition,callback,isSync){
	...
	var isDoQuery = $("#isDoQuery").val();
    if(!isDoQuery){
        isDoQuery = 0;
    }
	if(parseInt(isDoQuery) < 20){
		...
		$.ajax({
			...
			success: function(data){
				if(data.list.length > 0){
					if(callback){
						$("#isDoQuery").val(parseInt(isDoQuery)+1);
						callback(data);
					}
				}
			}
