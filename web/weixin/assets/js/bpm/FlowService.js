...
flowServiceModule.factory('flowService',['baseService', '$location', '$window', function(baseService, $location, $window){
	return {
		/**
		 * 获取发起表单界面
		 */
		...
		showOpinion:function(runId){
			var url=__ctx +"/weixin/bpm/opinionsHtml.html?runId="+runId;
		...
		showFlowPic:function(runId){
			var url=__ctx +"/weixin/bpm/bpmImage.html?runId="+runId;
			//var conf={};
			//conf.title="流程图"
			//conf.url=url;
		...
		updReaded:function(copyId){
			var url = __ctx+"/weixin/bpm/updReaded.ht?copyId="+copyId;
			baseService.get(url);
		}
	};
}])