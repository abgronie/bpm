...
app.controller('ctrl', ['$rootScope', '$scope', '$window', 'flowService',function($rootScope,$scope,$window,flowService){
	...
	$scope.startFlow=function(){
		...
		//手机表单点击提交按钮使用startFlowCallback方法
		if($window.startFlowCallback){
			conti = $window.startFlowCallback($scope);
		
			if(!conti.flag){
				Alert("提示信息",conti.msg);
				return;
			}
		}
